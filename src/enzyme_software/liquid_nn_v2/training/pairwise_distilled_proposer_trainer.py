...
if TORCH_AVAILABLE:
    ...
    class PairwiseDistilledProposerTrainer:
        ...
        def _run_batch(self, batch: Dict[str, object]):
            ...
            loss, distilled_metrics = self._distillation_loss(student_logits, payload)
            ...
            return loss, student_scores, distilled_metrics

        def _merge_distilled_epoch_metrics(self, accum: Dict[str, float]) -> Dict[str, float]:
            molecule_count = float(accum.get("distilled_molecule_count", 0.0))
            ...
            metrics["distilled_molecule_count"] = molecule_count
            if molecule_count <= 0.0:
                return metrics
            for key in (
                "distilled_kl_loss",
                ...
                metrics[key] = float(accum.get(f"{key}_sum", 0.0)) / molecule_count
            return metrics
        ...
        def train_loader_epoch(self, loader) -> Dict[str, object]:
            ...
            for raw_batch in loader:
                ...
                self.optimizer.step()
                molecule_count = float(batch_metrics.get("distilled_molecule_count", 0.0))
                for key, value in batch_metrics.items():
                    if key == "source_recall_at_6":
                        continue
                    if key == "skipped_singleton_candidate_molecules":
                        distilled_accum[key] = float(distilled_accum.get(key, 0.0)) + float(value)
                    elif key == "distilled_molecule_count":
                        distilled_accum[key] = float(distilled_accum.get(key, 0.0)) + molecule_count
                    else:
                        distilled_accum[f"{key}_sum"] = float(distilled_accum.get(f"{key}_sum", 0.0)) + (
                            float(value) * molecule_count
                        )
