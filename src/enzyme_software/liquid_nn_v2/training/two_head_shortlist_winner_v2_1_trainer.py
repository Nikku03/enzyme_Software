...
if TORCH_AVAILABLE:
    ...
    @dataclass
    class TwoHeadShortlistWinnerV2_1Trainer:
        model: object
        winner_head: object
        ...
        def _build_winner_features(
            ...
            *,
            ...
            second_score = selected_scores[1] if k > 1 else selected_scores[0]
            shortlist_mean = selected_scores.mean()
            shortlist_std = selected_scores.std(unbiased=False) if k > 1 else torch.zeros((), device=selected_scores.device, dtype=selected_scores.dtype)
            shortlist_std = shortlist_std.clamp_min(1.0e-6)
            shortlist_median = selected_scores.median()
            ...
            if bool(self.winner_v2_1_use_existing_candidate_features):
                feature_parts.append(selected_scores.unsqueeze(-1))
            if bool(self.winner_v2_1_use_score_gap_features):
                feature_parts.append((top_score - selected_scores).unsqueeze(-1))
            if bool(self.winner_v2_1_use_top2_gap_features):
                feature_parts.append((second_score - selected_scores).unsqueeze(-1))
            if bool(self.winner_v2_1_use_rank_features):
                if k > 1:
            ...
            if bool(self.winner_v2_1_use_normalized_score_features):
                feature_parts.append(normalized_score.unsqueeze(-1))
                feature_parts.append(((selected_scores - shortlist_mean) / shortlist_std).unsqueeze(-1))
            if bool(self.winner_v2_1_use_shortlist_context_features):
                candidate_count_feature = torch.full((k, 1), float(k), device=selected_scores.device, dtype=selected_scores.dtype)
                score_minus_mean = (selected_scores - shortlist_mean).unsqueeze(-1)
                score_minus_max = (selected_scores - shortlist_max).unsqueeze(-1)
                score_minus_median = (selected_scores - shortlist_median).unsqueeze(-1)
                top1_indicator = torch.zeros((k, 1), device=selected_scores.device, dtype=selected_scores.dtype)
                top2_indicator = torch.zeros((k, 1), device=selected_scores.device, dtype=selected_scores.dtype)
                top3_indicator = torch.zeros((k, 1), device=selected_scores.device, dtype=selected_scores.dtype)
                top1_indicator[0, 0] = 1.0
                if k > 1:
                    top2_indicator[1, 0] = 1.0
                if k > 2:
                    top3_indicator[2, 0] = 1.0
                feature_parts.extend(
                    [
                        candidate_count_feature,
                        score_minus_mean,
                        score_minus_max,
                        score_minus_median,
                        top1_indicator,
            ...
            if bool(self.winner_v2_1_use_pairwise_features):
                feature_parts.extend(
                    [
                        top_embedding,
            ...
            if bool(self.winner_v2_1_use_graph_local_features):
            ...
            if bool(self.winner_v2_1_use_3d_local_features):
            ...
            if int(winner_features.size(-1)) != int(self.winner_feature_dim):
                raise ValueError(
                    f"winner v2.1 feature dim mismatch: expected {self.winner_feature_dim}, got {int(winner_features.size(-1))}"
                )
            if not bool(torch.isfinite(winner_features).all()):
                raise FloatingPointError("Non-finite winner v2.1 features detected")
        ...
        def _build_winner_examples(self, atom_features, shortlist_scores, batch: Dict[str, object]):
            ...
            for mol_idx in range(num_molecules):
                ...
                target_index = -1
                if positive_count > 0:
                    if bool(self.winner_v2_1_use_soft_multi_positive_targets):
                ...
                examples.append(
                    {
                        ...
                        "trainable": bool((positive_count > 0) or (not self.winner_v2_1_train_only_on_hits)),
        ...
        def _winner_loss(self, examples):
            ...
            for example in trainable:
                ...
                if not bool(torch.isfinite(logits).all()):
                    raise FloatingPointError("Non-finite winner v2.1 logits detected")
                ...
                if target_distribution is not None:
                    ...
                    if not bool(torch.isfinite(target_distribution).all()):
                        raise FloatingPointError("Non-finite winner v2.1 target distribution detected")
                    losses.append(-(target_distribution * F.log_softmax(logits, dim=0)).sum())
                else:
            ...
            if not bool(torch.isfinite(loss)):
                raise FloatingPointError("Non-finite winner v2.1 loss detected")
            return loss
        ...
        def _run_batch(self, batch: Dict[str, object]):
            ...
            if atom_features is None:
                raise RuntimeError("two_head_shortlist_winner_v2_1 requires model outputs['atom_features']")
            if shortlist_logits is None:
                raise RuntimeError("two_head_shortlist_winner_v2_1 requires model outputs['site_logits']")
            ...
            total_loss = float(self.winner_v2_1_loss_weight) * winner_loss
            if not bool(torch.isfinite(total_loss)):
                raise FloatingPointError("Non-finite two-head v2.1 total loss detected")
            metrics = {
        ...
        def _finalize_epoch_metrics(
            ...
            *,
            ...
            if not shortlist_scores:
                raise RuntimeError("two_head_shortlist_winner_v2_1 received zero valid batches")
    ...
    @dataclass
    class TwoHeadShortlistWinnerV2_1Trainer:  # type: ignore[override]
