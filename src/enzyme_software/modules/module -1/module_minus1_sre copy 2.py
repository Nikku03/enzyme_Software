...
class NucleophileApproachCPT(CatalyticPerturbationTest):
    ...
    def run(
        ...
        """Geometric feasibility of nucleophile approach."""
        if fragment_mol.GetNumConformers() == 0:
            AllChem.EmbedMolecule(fragment_mol, randomSeed=0xBEEF)

        if fragment_mol.GetNumConformers() == 0:
            return {"error": "No conformer", "geometric_strain": None}
        ...
        try:
            ...
            return _fallback_geometric(fragment_mol=fragment_mol, attack_sites=attack_sites)
        ...
        c_atom = fragment_mol.GetAtomWithIdx(carbon_idx)
        plane_atoms = []
        for neighbor in c_atom.GetNeighbors():
            if neighbor.GetIdx() != oxygen_idx:
                plane_atoms.append(neighbor.GetIdx())

        angle_accessibility = 1.0
        if len(plane_atoms) >= 2:
            a_pos = conf.GetAtomPosition(plane_atoms[0])
            b_pos = conf.GetAtomPosition(plane_atoms[1])
            v1 = [a_pos.x - c_pos.x, a_pos.y - c_pos.y, a_pos.z - c_pos.z]
            v2 = [b_pos.x - c_pos.x, b_pos.y - c_pos.y, b_pos.z - c_pos.z]
            normal = _cross(v1, v2)
            norm = _norm(normal)
            if norm > 0:
                ...
                co_norm = _norm(co_vector)
                if co_norm > 0:
                    co_vector = [v / co_norm for v in co_vector]
                    attack_vector = _cross(co_vector, normal)
                    attack_norm = _norm(attack_vector)
                    if attack_norm > 0:
                        attack_vector = [v / attack_norm for v in attack_vector]
                        deviation = abs(_dot(normal, attack_vector))
                        angle_accessibility = 1.0 - min(deviation, 1.0)

        attack_point = rdGeometry.Point3D(
            c_pos.x + (o_pos.x - c_pos.x) * 2.0 / current_length,
            c_pos.y + (o_pos.y - c_pos.y) * 2.0 / current_length,
            c_pos.z + (o_pos.z - c_pos.z) * 2.0 / current_length,
        )

        try:
            steric_count = 0
            leaving_idx = attack_sites.get("leaving")
            for i in range(fragment_mol.GetNumAtoms()):
                if i in (carbon_idx, oxygen_idx, leaving_idx):
                    continue
                atom_pos = conf.GetAtomPosition(i)
                distance = math.sqrt(
                    (atom_pos.x - attack_point.x) ** 2
                    + (atom_pos.y - attack_point.y) ** 2
                    + (atom_pos.z - attack_point.z) ** 2
                )
                if distance < 3.5:
                    steric_count += 1

            steric_accessibility = max(0.0, 1.0 - (steric_count * 0.1))
            geometric_strain = max(
                0.0, 10.0 * (1.0 - angle_accessibility * steric_accessibility)
            )

            feasible = geometric_strain <= 5.0
            return {
                "geometric_strain": round(geometric_strain, 3),
                "angle_accessibility": round(angle_accessibility, 3),
                "steric_accessibility": round(steric_accessibility, 3),
                "current_carbonyl_length": round(current_length, 3),
                "energy_penalty_kcal": round(geometric_strain, 3),
                "method": "geometric_analysis",
                "direction": "penalty",
                ...
                "deltaE_rel": float(geometric_strain),
                "energy_penalty_rel": float(geometric_strain),
                "units": "rel",
                "geometry_feasible": bool(feasible),
            }
        except Exception:
            return _fallback_geometric(fragment_mol=fragment_mol, attack_sites=attack_sites)
...
class OxyanionHoleCPT(CatalyticPerturbationTest):
    ...
    def run(
        ...
        """Electronic readiness for oxyanion stabilization."""
        if fragment_mol.GetNumConformers() == 0:
            AllChem.EmbedMolecule(fragment_mol, randomSeed=0xBEEF)

        if fragment_mol.GetNumConformers() == 0:
            return {"error": "No conformer", "electronic_readiness": None}
        ...
        try:
            ...
            try:
                ...
                atom = fragment_mol.GetAtomWithIdx(oxygen_idx)
            ...
            for i in range(fragment_mol.GetNumAtoms()):
                if i == oxygen_idx:
                    continue
                atom = fragment_mol.GetAtomWithIdx(i)
                atom_pos = conf.GetAtomPosition(i)
                distance = math.sqrt(
                    (atom_pos.x - o_pos.x) ** 2
                    + (atom_pos.y - o_pos.y) ** 2
                    + (atom_pos.z - o_pos.z) ** 2
                )
                if distance < 4.0:
                    if atom.GetSymbol() in {"O", "N", "F", "Cl", "Br"}:
                        competing_groups += 1

            charge_score = min(1.0, max(0.0, -charge * 2.0))
            competition_penalty = min(1.0, competing_groups * 0.2)
            electronic_readiness = 10.0 * charge_score * (1.0 - competition_penalty)

            significant = electronic_readiness > 2.0
            return {
                "electronic_readiness": round(electronic_readiness, 3),
                "oxygen_charge": round(charge, 4),
                "competing_groups": competing_groups,
                "charge_score": round(charge_score, 3),
                "competition_penalty": round(competition_penalty, 3),
                "energy_gain_kcal": round(electronic_readiness, 3),
                "method": "electrostatic_analysis",
                "direction": "gain",
                "constraint_type": "ELECTROSTATIC",
                "deltaE_rel": float(electronic_readiness),
                "energy_gain_rel": float(electronic_readiness),
                "units": "rel",
                "stabilization_significant": bool(significant),
                "target_atoms": {"stabilizable": oxygen_idx},
            }
        except Exception:
            return _fallback_electrostatic(fragment_mol=fragment_mol, attack_sites=attack_sites)
...
def _attack_direction_from_carbonyl(
    ...
    """Calculate nucleophile attack vector at Bürgi-Dunitz angle (107°)."""
    # Vector FROM oxygen TO carbon (opposite of C=O bond)
    co_vec = [c_pos.x - o_pos.x, c_pos.y - o_pos.y, c_pos.z - o_pos.z]
    co_norm = _normalize(co_vec)
    if co_norm is None:
        return [1.0, 0.0, 0.0]
    
    # Get perpendicular vector for rotation
    ...
    # Rotate by Bürgi-Dunitz angle (107°)
...
def _build_probe_xyz_water(
    ...
    """Build XYZ with substrate + water probe without modifying the RDKit mol."""
    substrate_xyz = _mol_to_xyz(substrate_mol)
    if substrate_xyz is None:
        return None
    lines = substrate_xyz.strip().splitlines()
    if len(lines) < 2:
        return None
    try:
        n_substrate = int(lines[0].strip())
    except ValueError:
        return None
    ...
    angle = math.radians(52.25)  # half of 104.5° H-O-H angle
    oh_dist = 0.96
    h1_dir = [
        dir_norm[0] * math.cos(angle) + perp[0] * math.sin(angle),
        dir_norm[1] * math.cos(angle) + perp[1] * math.sin(angle),
        dir_norm[2] * math.cos(angle) + perp[2] * math.sin(angle),
    ]
    h2_dir = [
        dir_norm[0] * math.cos(angle) - perp[0] * math.sin(angle),
        dir_norm[1] * math.cos(angle) - perp[1] * math.sin(angle),
    ...
    h1_pos = (
        probe_pos.x + h1_dir[0] * oh_dist,
        probe_pos.y + h1_dir[1] * oh_dist,
        probe_pos.z + h1_dir[2] * oh_dist,
    )
    h2_pos = (
        probe_pos.x + h2_dir[0] * oh_dist,
        probe_pos.y + h2_dir[1] * oh_dist,
        probe_pos.z + h2_dir[2] * oh_dist,
    )

    n_total = n_substrate + 3
    new_lines = [str(n_total), lines[1]] + lines[2:]
    new_lines.append(
        f"O   {probe_pos.x:12.6f} {probe_pos.y:12.6f} {probe_pos.z:12.6f}"
    )
    new_lines.append(
        f"H   {h1_pos[0]:12.6f} {h1_pos[1]:12.6f} {h1_pos[2]:12.6f}"
    )
    new_lines.append(
        f"H   {h2_pos[0]:12.6f} {h2_pos[1]:12.6f} {h2_pos[2]:12.6f}"
    )
    return "\n".join(new_lines)
...
def _build_probe_xyz_ammonia_pair(
    ...
    """Build XYZ with substrate + two NH3 probes without modifying the RDKit mol."""
    substrate_xyz = _mol_to_xyz(substrate_mol)
    if substrate_xyz is None:
        return None
    lines = substrate_xyz.strip().splitlines()
    ...
    n_total = n_substrate + 8
    new_lines = [str(n_total), lines[1]] + lines[2:]
    for pos in (pos1, pos2):
        new_lines.append(
            f"N   {pos.x:12.6f} {pos.y:12.6f} {pos.z:12.6f}"
        )
        for vec in tet_vecs:
            new_lines.append(
                f"H   {pos.x + vec[0] * nh_dist:12.6f} "
                f"{pos.y + vec[1] * nh_dist:12.6f} "
                f"{pos.z + vec[2] * nh_dist:12.6f}"
            )
    return "\n".join(new_lines)
