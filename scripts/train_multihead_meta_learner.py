...
def main() -> None:
    require_torch()
    parser = argparse.ArgumentParser(description="Train the multi-head meta learner")
    ...
    parser.add_argument("--output-dir", default="checkpoints/meta_learner_multihead")
    parser.add_argument("--artifact-dir", default="artifacts/meta_learner_multihead")
    parser.add_argument("--cache-dir", default="cache/meta_learner_multihead")
    parser.add_argument("--mirank-weight", type=float, default=1.0)
    parser.add_argument("--bce-weight", type=float, default=0.3)
    parser.add_argument("--listmle-weight", type=float, default=0.0)
    parser.add_argument("--focal-weight", type=float, default=0.0)
    parser.add_argument("--ranking-margin", type=float, default=1.0)
    parser.add_argument("--hard-negative-fraction", type=float, default=None)
    ...
    if args.checkpoint:
        ...
        if int(cfg.get("hidden_dim", args.hidden_dim)) != int(args.hidden_dim):
            print(
                f"Requested hidden_dim={args.hidden_dim} ignored for resume; "
                f"using checkpoint hidden_dim={cfg.get('hidden_dim')}",
                flush=True,
            )
            model = MultiHeadMetaLearner(
                n_models=len(model_names),
        ...
        if skipped:
            print(f"Skipped incompatible checkpoint tensors: {len(skipped)}", flush=True)
    print(f"Multi-head meta-learner parameters: {sum(p.numel() for p in model.parameters()):,}", flush=True)

    config = MetaLearnerConfig(
        checkpoint_dir=args.output_dir,
        artifact_dir=args.artifact_dir,
        cache_dir=args.cache_dir,
        hidden_dim=model.site_attention[0].out_features,
    ...
    trainer = MultiHeadTrainer(model=model, train_dataset=train_dataset, val_dataset=val_dataset, config=config, device=_resolve_device(args.device))
    payload = trainer.train()
    print(f"Best validation Top-1: {payload['best_val_top1']:.3f}", flush=True)
    print(f"Final Site Attention: {payload.get('final_site_attention', [])}", flush=True)
    print(f"Final CYP Attention: {payload.get('final_cyp_attention', [])}", flush=True)
