...
# Import our dynamic state discovery engine
from dynamic_states import DynamicReverseGeometryEngine, DynamicStateLoss
...
class Config:
    ...
    hidden_dim: int = 128
    state_dim: int = 64
    max_states: int = 64  # Max states to discover
    similarity_threshold: float = 0.7  # Threshold for creating new state
    n_gnn_layers: int = 4
    dropout: float = 0.1
...
class CYP3A4Model(nn.Module):
    ...
    def __init__(self, config: Config, pocket: EnzymePocket):
        ...
        # Reverse geometry engine with dynamic states
        pocket_feat = pocket.get_pocket_tensor()
        self.register_buffer('pocket_feat', pocket_feat)
        
        self.reverse_geometry = DynamicReverseGeometryEngine(
...
def evaluate(model, loader, loss_fn, config):
    ...
    # Track state usage - use max_states size
    state_usage = torch.zeros(config.max_states)
    n_active_states = 0
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            state_probs = output['state_probs']  # (B, max_states)
            state_usage += state_probs.sum(dim=0).cpu()
            n_active_states = output.get('n_active_states', config.max_states)
    ...
    # Normalize state usage (only active states)
    if state_usage.sum() > 0:
...
def train(config: Config):
    print("="*70)
    print("CYP3A4 SoM Predictor v3 - Dynamic State Discovery")
    print("="*70)
    ...
    # Loss - use dynamic loss
    loss_fn = DynamicStateLoss()
    ...
    for epoch in range(config.epochs):
        ...
        # Print active states info
        n_active = val_metrics.get('n_active_states', 0)
        active_usage = val_metrics['state_usage'][:n_active] if n_active > 0 else val_metrics['state_usage']
        print(f"Active states: {n_active}, usage: {active_usage}")
